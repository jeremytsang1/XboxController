...
class XboxController(threading.Thread):
    ...
    def _setupPygame(self, joystickNo):
        # set SDL to use the dummy NULL video driver, so it doesn't
    ...
    def doCallBacks(self, control, value):
        ...
        if self.controllerCallBack is not None:
...
if __name__ == '__main__':
    ...
    # invert the Y Axis (for some reason in Pygame negative is up -
    # wierd!
    xboxCont = XboxController(controlCallBack, deadzone=30, scale=100,
                              invertYAxis=True)
